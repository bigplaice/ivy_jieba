...
char * 
ivyjieba_process_user_word(ArrayType *arr, bool add)
{
    ...
    if(!fp)
    {
        return("failed to open user dict file");
    }
    ...
    ndim = ARR_NDIM(arr);
    if (ndim != 1)
        return("only one-dimensional array is supported.");
    ...
    foreach(cell, entries)
    {
        ...
        foreach(ecell, existing)
        {
            ...
            if (strcmp(de->word, e->word) == 0)
            {
                if(!add)
                {
                    // found matched word, delete it.
                    list_delete_cell(existing, ecell);
    ...
    if (updated_count > 0 || added_count > 0)
    {
        ...
        if (!out)
             return("can not write into dict file");
    ...
    if(add)
        ...
        appendStringInfo(&output, "operation completed: delete %d", deleted_count);
...
ivyjieba_add_user_word(PG_FUNCTION_ARGS)
{
	...
	if(0 == ARR_NDIM(arr))
	{
		PG_RETURN_TEXT_P(cstring_to_text("no word provided."));
	}

	ret_str = ivyjieba_process_user_word(arr, true);
	PG_RETURN_TEXT_P(cstring_to_text(ret_str));
}

Datum
ivyjieba_del_user_word(PG_FUNCTION_ARGS)
{
	ArrayType *arr;
	char *ret_str;
